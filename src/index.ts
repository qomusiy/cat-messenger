...
const openScriptConsole = async() => {
  const {showScriptConsole} = await import('./components/scriptConsole/trigger');
  showScriptConsole();
};

(window as any)['openScriptConsole'] = openScriptConsole;

// Ctrl/Cmd + Shift + K — same slot the browser devtools console uses, so it
// reads as "the console" without colliding with any in-app shortcut.
