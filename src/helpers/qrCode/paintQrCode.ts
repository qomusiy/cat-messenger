...
// The logo SVG is identical for a given tint every paint; fetch + recolor +
// data-URL it once per colour instead of on every QR regenerate.
// NOTE: the Cat Messenger mark is a deliberate two-tone black/white brand asset
// and carries no `fill:...;` style declaration, so the recolor below is a no-op
// for it and the mark stays brand-coloured. `logoColor` is kept because the
// replace still applies to any single-tint logo swapped in here later.
