...
export class AppImManager extends EventListenerBase<{
  ...
  private attachKeydownListener() {
    ...
    const onKeyDown = async(e: KeyboardEvent) => {
      ...
      // NB: TEXTAREA counts too — without it every keystroke typed into a textarea
      // falls through to the "type anywhere → focus the message input" block below
      // and gets forwarded to the chat input instead. Matches the check in
      // useLockScreenShortcut.
      const isTargetAnInput = (target.tagName === 'INPUT' && !['checkbox', 'radio'].includes((target as HTMLInputElement).type)) ||
